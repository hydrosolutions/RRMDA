...
if set>0
    try
        ...
        try
            [A] = geotiffread(strcat(regFile, datestr(inDate,'yy'),setStr, 'm.tif'));
            R = worldfileread(strcat(regFile, datestr(inDate,'yy'),setStr, 'm.tfw'),'geographic',size(A));
        catch
            try
                [A] = geotiffread(strcat(regFile, setStr, datestr(inDate,'yy'),'.tif'));
                R = worldfileread(strcat(regFile, setStr, datestr(inDate,'yy'),'.tfw'),'geographic',size(A));
            catch
                [A] = geotiffread(strcat(regFile, datestr(inDate,'yy'),setStr, '.tif'));
                R = worldfileread(strcat(regFile, datestr(inDate,'yy'),setStr, '.tfw'),'geographic',size(A));
            end
        end
    end        
else
