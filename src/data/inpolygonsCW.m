function [in, index] = inpolygonsCW(x,y,xv,yv)
...
% isCw = ispolycw(xsplit, ysplit); if we know that polygons are already CW
% - we can reduce overhead by not checking
% isCw = ones(size(xsplit,1),1)==1;
le = length(xsplit);
mainPolyIndices = (1:le)';
nHolesPer = diff([mainPolyIndices;le+1]) - 1;
