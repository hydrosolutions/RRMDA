...
sites = [13,14,15,16,19,20,21,22,23,25,26,37,43,44,45,48,49,50,53,56,57,58,59,60,61,62,63,64,65,66];
...
id17 = find([values.siteID]==44); % Mosses, Mwandeti furrow
count17 = length(values(id17));

id18 = find([values.siteID]==48); % Clemens, Urangini Furrow
count18 = length(values(id18));

id19 = find([values.siteID]==49); % Clemens, Urangini River
count19 = length(values(id19));
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
data{18,1}='44';data{18,2}='Mosses';data{18,3}='255654106858';data{18,4}='Mwandeti Furrow';

data{19,1}='48';data{19,2}='Clemens';data{19,3}='255752095454';data{19,4}='Urangini Furrow';

data{20,1}='49';data{20,2}='Clemens';data{20,3}='255752095454';data{20,4}='Urangini River';
...
if ~tobiasTest
    recipient = {'rnaudascher@yahoo.de','siegfried@hydrosolutions.ch',...
        ...
        'patrice.mueller@he-arc.ch'};
else
    recipient = {'siegfried@hydrosolutions.ch'}; % testing
end
...
message=['Yesterday the following data was submitted:' 10 ...
    ...
    'Site ID' ' ' data{14,1} ': ' data{14,2} ' ' 'at' ' ' data{14,4} ' ' 'sent' ' ' num2str(data{14,end}) ' ' 'values' 10 ...
    'Site ID' ' ' data{16,1} ': ' data{16,2} ' ' 'at' ' ' data{16,4} ' ' 'sent' ' ' num2str(data{16,end}) ' ' 'values' 10 ...
    'Site ID' ' ' data{17,1} ': ' data{17,2} ' ' 'at' ' ' data{17,4} ' ' 'sent' ' ' num2str(data{17,end}) ' ' 'values' 10 ...
    'Site ID' ' ' data{18,1} ': ' data{18,2} ' ' 'at' ' ' data{18,4} ' ' 'sent' ' ' num2str(data{18,end}) ' ' 'values' 10 ...
    'Site ID' ' ' data{19,1} ': ' data{19,2} ' ' 'at' ' ' data{19,4} ' ' 'sent' ' ' num2str(data{19,end}) ' ' 'values' 10 ...
    'Site ID' ' ' data{20,1} ': ' data{20,2} ' ' 'at' ' ' data{20,4} ' ' 'sent' ' ' num2str(data{20,end}) ' ' 'values'];
